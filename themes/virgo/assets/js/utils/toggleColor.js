// Nhập DarkReader, thực hiện chuyển đổi chế độ sáng tối của theme
...
// Khởi tạo màu theme (hiện tại chưa thiết lập lưu trữ cục bộ `dark`)
// ❌ Bỏ dùng, mặc định thiết lập màu sáng
// if (!getDarkOfLocalStorage()) {
//     enable(initColor);
//     setDarkOfLocalStorage('on');
// }
// ✔️ Cấu hình khởi tạo màu mới
if (DARK && getDarkOfLocalStorage() !== 'off') {
    setDarkOfLocalStorage('on');
}

// Nếu tồn tại lưu trữ cục bộ và trạng thái:
// là `on`, kích hoạt chế độ tối,
// là `off`, kích hoạt chế độ sáng
